...
#pragma endregion
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		try {
			...
			connect->Open();
			String^ query = "SELECT * FROM login1 WHERE email = @username AND password = @password";
			...
			if (reader->HasRows) {
				// Login successful
				OOPFINALPROJECTSEMESTER2::dashboard^ ds = gcnew OOPFINALPROJECTSEMESTER2::dashboard();
				ds->Show();
				this->Hide();
